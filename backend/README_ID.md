...
Aplikasi backend E-commerce menggunakan **Laravel 11** dengan arsitektur **Domain-Driven Design (DDD)**, dukungan multi-tenancy, dan database SQLite.

## 📚 Daftar Isi

- [🏗️ Arsitektur](#-arsitektur)
- [📁 Struktur Project](#-struktur-project)
- [⚙️ Setup & Instalasi](#-setup--instalasi)
...
Aplikasi mengikuti pola **DDD** dengan pemisahan jelas antara 3 layer:
...
- Validasi input data (Form Requests)
- Orchestration logika aplikasi
- Format dan kembalikan API responses
...
- Mendefinisikan repository interfaces
...
**Tanggung Jawab:**
- Implementasi concrete dari repository interfaces
- Database queries dan logic
- Cache management
...
Frontend dapat mengakses dari:
- `http://localhost:5173` (Vite dev server)
- `http://localhost:5174` (Fallback port)

### Sanctum Token Configuration

API authentication menggunakan Laravel Sanctum. Token konfigurasi:
...
**Roles tersedia:**
- `admin` - Full access untuk manage products
- `customer` - Regular user untuk shopping
...
Semua migrations di-manage via Artisan commands:
...
File seeder: `database/seeders/DatabaseSeeder.php`

Test data yang di-generate:
- **1 Admin User** (credentials: admin@example.com / password)
- **3 Customer Users** (customer1@example.com, customer2@example.com, etc.)
- **10 Products** (dengan various categories dan prices)
...
Return: { token, user_data, roles }
    ↓
Store token di frontend (localStorage)
    ↓
Send token dalam Authorization header untuk request berikutnya
```
...
**Available Roles:**
- `admin` - Dapat manage products, view analytics
- `customer` - Dapat browse products, cart, checkout
...
// Protected route - hanya authenticated users
